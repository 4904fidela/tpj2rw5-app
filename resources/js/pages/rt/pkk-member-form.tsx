...
export default function PkkMemberForm({ auth, residents, member }: Props) {
    ...
    return (
        <RtLayout
            title={member ? 'Edit Anggota PKK' : 'Tambah Anggota PKK'}
            auth={auth}
        >
            <Head
                title={`${member ? 'Edit' : 'Tambah'} Anggota PKK - ${auth.user.rt.name}`}
            />

            <Card className="max-w-2xl">
                <CardHeader>
                    <CardTitle>
                        {member ? 'Edit Anggota PKK' : 'Tambah Anggota PKK'}
                    </CardTitle>
                </CardHeader>
                <CardContent>
